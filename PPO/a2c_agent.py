...
class A2CAgent:
    ...
    def append_sample(self, state, action, reward,point,preds):
    ...
    def act(self, state, init=False):
        ...
        #add value fake input
        state.append(np.zeros((1,1)))
        #add old policy pred fake input
        state.append(np.zeros((1,len(self.categorical_actions)+len(self.spatial_actions))))
        #add old spatial pred fake input
        state.append(np.zeros((1,4096)))

        preds = self.model.predict(state)
        return self.action_from_id[np.random.choice(len(self.action_from_id),1,p=preds[1][0])[0]],np.random.choice(4096, 1, p=preds[2][0])[0],preds
    ...
    def train(self):
        ...
        update_inputs = [np.zeros((episode_length, 17, 64, 64)),
                         ...
                         np.zeros((episode_length, len(self.categorical_actions) + len(self.spatial_actions))),
                         # add old spatial pred fake input
                         np.zeros((episode_length, 4096))
						 #np.zeros((episode_length, 1))
        ...
        for i in range(episode_length):
            ...
            update_inputs[3][i, :] = self.predictions[2][1][0, :]
            update_inputs[4][i, :] = self.predictions[2][2][0, :]
        ...
        self.model.fit(update_inputs, [discounted_rewards, advantages_actions,advantages_space], epochs=3, verbose=0)
